...
*  **I.** Since `Chairs`, `Printers`, and `Phones` all bring in roughly the same gross revenue due to the uniform split, stocking and selling the items that are the cheapest to warehouse and easiest to ship (like `Phones` over bulky `Desks`) should be prioritized to maximize net profit margins.
